...
homo_dir x (Star r1) = Star (homo_dir x r1)

-- Inverse homomorphic image
homo_inv :: Eq a => (Char -> String) -> FSM a -> FSM a
homo_inv x (qs1, s1, fs1, d1) = (qs, s, fs, d) where
  qs = qs1
  s = s1
  fs = fs1
  d q a = star d1 q (x a)
...
quot_right xs (qs1, s1, fs1, d1) = (qs, s, fs, d) where
  qs = qs1
  s = s1
  fs = [q | q <- qs1, or [elem (star d1 q w) fs1 | w <- xs]]
  d = d1
...
nap_hat_star d qs (a:as) = nap_hat_star d (nap_hat d qs a) as

-- nacc m w == m accepd the string w
nacc :: Ord a => NFSM a -> [Char] -> Bool
nacc m@(qs,s,fs,d) w = overlap (nap_hat_star d s w) fs
...
eclose es qs = uclosure qs (\x -> [q' | (q,q') <- es, q == x])
...
eap_hat_star (d,es) qs []     = qs
eap_hat_star (d,es) qs (a:as) = eap_hat_star (d,es) (eap_hat (d,es) qs a) as

eacc :: Ord a => EFSM a -> [Char] -> Bool
eacc m@(qs,s,fs,d,es) w = overlap (eap_hat_star (d,es) s w) fs
...
efsm_to_fsm (qs1,s1,fs1,d1,es) = (qs,s,fs,d) where
  qs = [x | x <- power (eclose es qs1)]
  s = s1
  fs = [x | x <- qs, overlap x fs1]
  d q a = eap_hat (d1,es) q a
