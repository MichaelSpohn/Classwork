...
subset [] _ = True
subset (x:xs) ys
         | element x ys == True = subset xs ys
         | otherwise            = False
...
inters [] ys = []
inters (x:xs) ys
         | element x ys = x : inters xs ys
         | otherwise    = inters xs ys
...
differ [] ys = []
differ (x:xs) ys
         | element x ys = differ xs ys
         | otherwise    = x : differ xs ys
...
cart _ [] = []
cart [] _ = []
cart (x:xs) (y:ys) = [(x,y)] ++ cart [x] ys ++ cart xs ys ++ cart xs [y]
